...
% This surrogate will be constructed as a linear estimation:
...
%   m_{\Delta}(\kk,\uu) &= m_Z(\kk,\uu) - \alpha m_Z^*(\uu) \label{eq:mu_delta_GP}\\
%   \sigma^2_{\Delta}(\kk,\uu) &= \sigma_Z^2(\kk,\uu) + \alpha^2 \sigma_{Z^*}^2(\uu) - 2\alpha C\left((\kk,\uu),(\kk^*(\uu),\uu)\right) \label{eq:variance_delta_GP}
% \end{align}
