...
\end{equation}
Based on this conditioned GP \todo{finir} 
...
For the plug-in approach, the chosen estimator is defined \cref{eq:def_gamma_PI}:
  ...
  Let $\hat{\Gamma}_{\alpha,n}$ be the plug-in approximation of $\Gamma_\alpha$, constructed using the Gaussian Process surrogate with $n$ points added, according to the augmented IMSE. \Cref{fig:IMSE_enrichment} illustrate the $L^2$ and $L^{\infty}$ between the truth $\Gamma_\alpha$ and the estimation $\hat{\Gamma}_{\alpha,n}$.
