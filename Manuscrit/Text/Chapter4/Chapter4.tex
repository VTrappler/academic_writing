...
and evaluating the maximizers of this criterion is not really
relevant.

Instead, by integrating $\mathscr{V}_A$ over the whole space
$\Xspace$, we get a measure of the uncertainty in the classification
problem associated with the design $\mathcal{X}_n$, as the criteria
introduced~\cite{bect_sequential_2012} and what has been introduced
for the $\IMSE$, which we will denote $\mathrm{IVPC}(\mathcal{X}_n)$
...
of~\cref{alg:SUR_strat}, and sampling in regions of large
uncertainties, as detailed~\cref{alg:sampling_enrichment}.
...
optimisation problem,~\cite{miranda_adjoint-based_2016} introduce a
...
In the previous chapter, we introduced the conditional minimum
...
where a slight abuse of notation $J(\kk, \uu) = f(x)$ with $x=(\kk, \uu)$ and thus $\mathcal{X}_0 = \{(x_i, f(x_i)\}_{1\leq i\leq n_0}$
...
\cref{alg:SUR_strat}, where at each iteration, the PEI is maximized
...
\begin{figure}[ht]
  ...
  criterion]{\label{fig:example_PEI} GP after 30 additional iterations
    ...
    represented on the top left plot, and the PEI criterion for this
    GP is evaluated over the whole space is plotted top right. Bottom
...
\item Assuming a stationary form of the covariance, the third term is
  dependent only on the distance between $\kk$ and $\kk^*_Z*(\uu)$. By
  separating the $\kk$ and $\uu$ components, the covariance term can
  be written
...
Let consider $\alpha\geq 1$ fixed. In order to compute $\estimtxt{\kk}{RR, \alpha}$, we need to estimate and optimize the function $\Gamma_{\alpha}$. For that purpose, we are going to explore the space to improve the estimation of $\Gamma_{\alpha}$, and once sufficient knowledge is acquired, use the plug-in % \todo{definir}
...
$Z - \alpha Z^*$ as a learning function. The choice of the IMSE
...
IMSE.~\Cref{fig:IMSE_enrichment} illustrates the $L^2$ and
...
As mentioned before, the uncertainty on the acceptability of those
points can be reduced either by evaluating the function at this point
(thus reducing $\sigma_Z$), or by improving the knowledge of the
conditional minimiser (reducing $\alpha^2 \sigma_{Z^*}$). For each
centroid $\tilde{x}_{n+i} = (\tilde{\kk}_{n+i}, \tilde{\uu}_{n+i})$,
we can then adjust its position if the uncertainty on the conditional
minimiser is large, by introducing an adjustment function:
\begin{equation}
...
for the point which has the smallest $\sigma^2_Z$. In the end, the
points added to the design are the points labeled ``Adjusted centroids
2''. This double adjustment is described
schematically~\cref{fig:schema_double_adj}, and is applicated on the
real function~\cref{fig:adjusted_centroids}.
\begin{figure}[ht]
  \centering
  \input{\imgpath schema_double_adjustment.pgf}
...
selecting points using the sampling based methods described \cref{alg:sampling_enrichment_star}}
\end{figure}
...
As evoked in~\cref{sec:choice_threshold} we can instead look to tune
...
Every step of this method require the optimization of the expected
...
value that the minimal quantile may
take. % The set $\{\mathfrak{q}_l\}$ can be considered as plausible
      % values of the wanted quantile $q_p$. \todo{choice of
      % $\tilde{\kk}$, for discretization of quantiles of interest}
...
adaptative strategy does not take into account the subsequent
...
order to apply the algorithms based on GP, the dimension will first
...
of a realistic numerical model.
