...
\begin{definition}[Random process]
  ...
  A random process $Z$ is a collection of random variables indexed on $\Xspace$, so for each $x \in \Xspace$, $Z(x)$ is a real random variable (\textit{i.e.} a mapping from $\Omega$ to $\mathbb{R}$% \todo{notation?}
  ):
 \begin{equation}
  \begin{array}{rcl}
...
\end{equation}
\Cref{fig:prob_coverage_example} shows the probability of coverage of the set $A$, the true set $f^{-1}(B)$, and the plugin estimation of this set: $m_Z^{-1}(B) = \{x\in\Xspace \mid m_Z(x) \leq T\}$.
For a given $x\in\Xspace$, the event ``$x$ belongs to $A$'' happens with probability $\pi_A(x)$, thus has variance %\todo{expliciter Loi de bernoulli}
\begin{equation}
...
\end{equation}
and the margin of uncertainty of level $\eta$ is \todo{recalling}
\begin{equation}
...
enough. % \todo{Plus de rappels, points d'étapes}
