...
A few common stationary covariance functions are introduced~\cref{tab:common_cov_fc}.
  ...
  These covariance function introduce an additional parameter $l$, called the \emph{length scale}.
...
\subsection{Estimation of regret based estimators}
...
\subsubsection{Evaluation and optimization of $\Gamma$}
\paragraph{Estimation of the probability of coverage}
...
\begin{equation}
  \hat{\Gamma}_{\alpha,n}(\kk) = \Ex_U\left[\pi_{\alpha}(\kk,\uu)\right]=\int_{\Uspace}\pi_{\alpha}(\kk,\uu)p_{\UU}(\uu) \,\mathrm{d}\uu = \int_{\Uspace}\Phi\left(-\frac{m_{\Delta_\alpha}(\kk,\uu)}{\sigma_{\Delta_\alpha}(\kk,\uu)}\right)p_{\UU}(\uu) \,\mathrm{d}\uu
\end{equation}
