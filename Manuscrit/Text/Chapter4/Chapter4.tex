...
for numerical stability, we can add a \emph{nugget} effect to $\mathbf{K}_{\mathcal{X}}$.~\eqref{eq:GP_joint_distrib_nugget} becomes:
\begin{equation}
...
The $(\dim \Xspace + 1)$ hyperparameters of the covariance: the length scales $(l_1,\dots,l_{\dim \Xspace})$ and the variance parameter of $s$ have to be estimated based on the training set $\mathcal{X}$. This is usually done by MLE~(see for instance~\cite{ribaud_robustness_2019}), or by cross-validation (see~\cite{ginsbourger_note_2009}).
...
We detailed so far the construction of a GP on a unknown function $f$, based on a initial design $\mathcal{X}_0$. However the surrogate is usually not an end in itself. This surrogate is used afterward to replace the original function for expensive procedures.
By adding points to the design, we can improve the accuracy of the surrogate in some regions of of interest, depending on the application foreseen, such as optimization. In~\cref{sec:robust_criteria_gp}, we will introduce other criteria that focus on the robust estimation, to solve the problems introduced in the previous chapter.

Choosing the points to add to the design can be done optimally, one by one, as described \cref{ssec:1stepSUR}, or we can look to add a batch of $q$ points each iteration. 
...
Due to the structure of many computer codes, it may sometimes be beneficial to evaluate a batch of points, instead of selecting a unique point at each step of the procedure. The SUR strategies introduced before require the optimization of a criterion function to select a single point, but in~\cite{ginsbourger_kriging_2010}, the authors introduce a few ways to transform those criteria in order to be able to select $q$ points that will be evaluated by the true function.

% \paragraph{Transforming a 1-step into a $q$-step criterion}
%  Using a greedy approach, we can modify $1$-step criteria to $q-$step criteria, by performing successive optimisation. Let $\kappa$ be an enrichment criterion, $\mathcal{X}_n$ the design at the iteration $n$, and we define for $1\leq i \leq q$, 
% \begin{align}
...
% \end{align}
% Each point $x^{n+i}$ is associated with $z^{n+i}$, a \emph{potential} evaluation of $f(x^{n+i})$, \emph{i.e.} a $z^{n+i}$ is a realization of $Z(x^{n+i})$.
% Depending on the choice of the realizations $z^{n},\dots,z^{n+i-1}$. We can augment the design in different ways:
% \begin{itemize}
...
% \end{itemize}
% By choosing a specific realization instead of averaging over all possible realizations, we avoid the avoid the successive optimization of expectations, which can be very expensive.
% However, despite avoiding the successive integrals, the optimizations can be expensive.
...
\STATE Get the $q$ centroids $\{x_j^{\mathrm{center}}\}_{1 \leq j \leq q}$ using KMeans on $S$
...
\begin{equation}
  x_{n+1} = \argmax_{x\in \Xspace} \kappa(x) = \argmax_{x \in \Xspace} \sigma^2_{Z \mid \mathcal{X}}(x)
\end{equation}
...
For a given $x\in \Xspace$ and an outcome $z=f(x)\in \mathbb{R}$, the augmented design is defined as $\mathcal{X} \cup \left\{(x, z)\right\}$, and the IMSE of the augmented design is $\IMSE\left(Z \mid \mathcal{X} \cup \left\{(x, z)\right\} \right)$.
...
In this section, we are interested in the estimation of the inverse image of sets: $\{x \in \Xspace \mid f(x) = T\}=f^{-1}(\{T\})$ for level sets, or $\{x\in\Xspace \mid f(x) \leq T\} = f^{-1}(]-\infty, T])$ called excursion sets. An exemple of this estimation is illustrated \cref{fig:prob_coverage_exemple}, where the focus is on the estimation of the set $f^{-1}(\interval[open left]{-\infty}{T})$. For that, we are going to introduce enrichment strategies which rely on the evalution of the function around the level set $\{f=T\}$.

\subsubsection{Probability of coverage}
We can be interested in the estimation of a volume given by an unknown function. Given $\Prob_{X}$ a probability measure on $\Xspace$, and $B\subset \mathbb{R}$, we can compute $V=\Prob_{X}\left[f^{-1}(B)\right]$. For $B = \interval[open left]{-\infty}{T}$ is equivalent to compute the volume of the \emph{excursion set} of $f$ below $T$: $V=\Prob_{X}[f(X) \leq T]$.

\begin{figure}[ht]
  \centering
  \input{\imgpath prob_coverage_exemple.pgf}
  \caption{\label{fig:prob_coverage_exemple} Estimation of $f^{-1}(\interval[open left]{-\infty}{T})$ using GP and probability of coverage associated}
\end{figure}
...
\end{equation}
On \cref{fig:prob_coverage_exemple} shows the probability of coverage of the set $A$, the true set $f^{-1}(B)$, and the plugin estimation of this set $m_Z^{-1}(B) = \{x\in\Xspace \mid m_Z(x) \leq T\}$.
...
For a function $f$, let us assume that we are interested in the estimation of the set $f^{-1}(B) = \{x \in \Xspace \mid f(x) \leq T\}$, with  $B = ]-\infty, T]$.
...
Using the properties of the GP, we can express the probability of coverage of the random set $A$ using $F_{Z(x)}$, the cdf of the r.v.\ $Z(x)$, or $\Phi$, the cdf of the centered standard Gaussian distribution:
\begin{align}
  ...
  \mathscr{P}_{\mathrm{mis}}(x) &= \min\left(F_{Z(x)}(T), 1-F_{Z(x)}(T)\right)=\min\left(F_{Z(x)}(T),F_{-Z(x)}(-T)\right) \\
           &=\min\left(F_{Z(x)}(T),F_{Z(x)-2m_{Z}(x)}(-T)\right)  \\ &= \min\left(\Phi\left(-\frac{m_Z(x) - T}{\sigma_Z(x)}\right), \Phi\left(-\frac{-m_Z(x) + T}{\sigma_Z(x)}\right)\right) \\
           &= \min\left(\Phi\left(-\frac{m_Z(x) - T}{\sigma_Z(x)}\right),\Phi\left(+\frac{m_Z(x) - T}{\sigma_Z(x)}\right)\right)\\
...
First, we can minimize the point which has the maximal probability of misclassification, or equivalently since $\Phi$ is monotonously increasing, maximize the criterion $\kappa_{\rho}$ given by
\begin{equation}
  x_{n+1} = \argmin_{\kk\in\Kspace} \rho(x) =\argmax_{\kk\in\Kspace} -\frac{\lvert m_Z(x) - T \rvert}{\sigma_Z(x)}
\end{equation}
...
\begin{equation}
  ...
  x_{n+1} = \argmin_{\kk \in\Kspace} \Ex_{Z(x)}\Big[\mathrm{IVPC}(\mathcal{X}_n \cup \{   \left(x, Z(x) \right)  \})\Big]
\end{equation}
...
\section{Adaptative strategies of GP for robust optimisation}
...
A surrogate of $J$, constructed using the GP $Z$ is then $m_Z$, which will be used to compute the different estimates. However, the initial design $\mathcal{X}_0$ is probably too sparse to get $m_Z$ accurately enough to compute the wanted estimates. For that purpose, we propose an enrichment strategy in order to add points to the design $\mathcal{X}_0$. As in the previous section, the enrichment criterion is dependent on the 

One main challenge when making the distinction $\Xspace = \Kspace \times \Uspace$ is that $\kk$ and $\uu$ play different roles: One need to explore sufficiently the space $\Uspace$ according to the distribution of $\UU$, in order to be able to compute $\Prob_{\UU}$ and $\Ex_{\UU}$. On the other hand, not all $\kk \in\Kspace$.

While some methods introduced here involve the computation and the maximization of a criterion on the joint space directly, following the method of~\cref{alg:SUR_strat}% , some others use the GP $Z$ to remove the dependence on $\UU$, by projecting the GP for instance 
...
\subsection{Conditional optimisers: the Profile Expected Improvement}
...
\begin{equation}
  \max_{\uu\in\Uspace} \sigma^2_Z(m_{Z^*}(\uu), \uu) \leq T
\end{equation}
...
\subsection{GP modelling of the regret}
...
The strategies introduced hereafter can be easily be adapted for the additive-regret, by setting $\alpha=1$, and translating the mean functions accordingly, thus defining $\{\Delta_{\alpha=1} \leq \beta \}$. The 
...
\subsection{Evaluation of the acceptability probability $\Gamma$}
...
Based on those two approximation, we can define two different estimations of $\Gamma_\alpha$, namely $\hat{\Gamma}_\alpha^{\mathrm{PI}}$ with the plug-in approach, and $\hat{\Gamma}_{\alpha}^{\pi}$ for the probabilistic one.
\begin{itemize}
...
\begin{figure}[ht]
  \centering
  % \input{\imgpath IMSE_enrichment_dist_to_objective.pgf}
  \caption{\label{fig:IMSE_enrichment_ptarget} Squared distance between the true value of $\max\Gamma_{\alpha}$ and the estimated maximum using probabilistic $(\pi)$ and plug-in approach $PI$}
\end{figure}

  
\subsubsection{The probability of coverage}
\paragraph{Reducing the augmented IVPC}
...
\begin{figure}[ht]
  ...
  \caption{\label{fig:prob_coverage_margin} Probability of coverage and Margin of uncertainty for  $\{\Delta_{\alpha} \leq 0\}$ at level $\eta=0.05$}
\end{figure}
...
Let us assume that the current design used to construct $Z$ comprises $n$ points, and let us assume that we obtained $q$ points which represent statistically the margin of uncertainty: $\tilde{x}_{n+j}$ for $1\leq j\leq q$. Those points being in our case the centroids produced by the KMeans algorithms.
As mentioned before, the uncertainty on the acceptability of those points to  can be reduced either by evaluating the function at this point (thus reducing $\sigma_Z$), or by improving the knowledge of the conditional minimiser (reducing $\alpha^2 \sigma_{Z^*}$). For each centroid $\tilde{x}_{n+j} = (\tilde{\kk}_{n+j}, \tilde{\uu}_{n+j})$, we can then adjust its position if the uncertainty on the conditional minimiser is large. We introduce then a function to adjust those points in~\cref{eq:adjust_centroid}.
\begin{equation}
...
\begin{figure}[ht]
  ...
  \caption{\label{fig:adjusted_centroids} Sampling in the margin of uncertainty, and adjustment of centroids. }
\end{figure}


However, as we can see on the left plot of~\cref{fig:adjusted_centroids}, all centroids are adjusted independently, and since only their $\kk$ component is changed, the adjusted centroids may end up very close to each other. The issue is twofold: the evaluation of those points brings redundant information to the problem, and thus we do not take fully advantage of the batch evaluations.
Moreover when adding  to the design, the proximity can lead to an ill-conditioned covariance matrix of the GP and in turn numerical problems in the estimation of its hyperparameters.


This can be patched by setting a minimal distance between the wanted evaluation points $\varepsilon>0$.
Let $x_1,\dots, x_d$ be $d$ adjusted centroids which are too close to each other, \textit{i.e.} verifying $\max_{i,j}\|x_i - x_j\| \leq \varepsilon$. We can perform once again a clustering procedure, this time adapted to lower sample size as the number of centroids is limited, such as Hierarchical clustering~\cite{nielsen_hierarchical_2016}.
As they are in close proximity, we can assume that the uncertainty on the conditional minimiser $\alpha^2\sigma^2_{Z^*}$ is sensibly the same. So we are going to adjust only the one which has the smallest $\sigma^2_Z$.
...
\FOR{$j=1$ to $q$} % \COMMENT{Find the $N_s$ closest points}
\STATE $\tilde{x}_{n+j}=(\kk_{n+j},\uu_{n+j})=\argmin_{x \in S \setminus \{x_{n+1}, \dots, x_{n+j-1}\}} \|x - x_j^{\mathrm{center}} \|$
\ENDFOR

\STATE Hierarchical clustering of $\{\mathrm{adjust}(\tilde{x}_{n+j})\}$ for $1 \leq j \leq q$
\STATE Readjusted centroids are $\{x_{n+j}\}_{1\leq j\leq q}$
\STATE Evaluate $f(x_{n+j})$ for $A\leq j \leq q$
\STATE $\mathcal{X}_{n+q} \leftarrow \mathcal{X}_n \cup \left\{\left(x_{n+1}, f(x_{n+1})\right),\dots,  \left(x_{n+q}, f(x_{n+q})\right)\right\}$
\STATE Condition the GP according to $\mathcal{X}_{n+q}$
\STATE $n \leftarrow n + q$
\ENDWHILE
...
\subsection{Estimation of $\alpha_p$ as a quantile of the relative regret}
...
Let us recall that for a level of confidence $p$, we define $\alpha_p$ as the smallest threshold giving a maximal probability of acceptability above $p$.
\begin{equation}
...
\begin{align}
  ...
  \iff &p          = \Prob_{\UU}\left[\frac{J(\kk, \UU)}{J^*(\UU)} \leq q_{p}(\kk)\right] = \Gamma_{q_p(\kk)}(\kk)
\end{align}
...
As this formulation relies on quantiles, we are first going to detail a bit their estimation.
...
%   ????
% \end{equation}
Once again, using the GP $Z$ constructed on $J$, more specifically its prediction or its sample paths, we can construct a 
\paragraph{Lognormal approximation of the ratio}
Let $\kk\in\Kspace$ and $\uu\in\Uspace$.
The true value of the ratio $J(\kk,\uu)/J^*(\uu)$ is modelled as the random variable $Z(\kk,\uu)/Z^*(\uu)$, where
\begin{equation}
  \begin{bmatrix}
    Z(\kk, \uu) \\ Z^*(\uu)
  \end{bmatrix}
...
\begin{align}
  ...
  &\sim \log\mathcal{N}(m_{\nu}(\kk, \uu), \sigma^2_{\nu}(\kk, \uu))
\end{align}

We can then define the plugin estimation of the ratio as the mean value of the ratio:
\begin{equation}
  \label{eq:mean_ratio_logn}
  \Ex_Z\left[\frac{Z(\kk, \uu)}{Z^*(\uu)}\right] = \frac{m_Z(\kk, \uu)}{m_{Z^*}(\uu)}\exp\left(\frac{1}{2}\sigma^2_{\nu}(\kk, \uu)\right) 
\end{equation}
and then use \cref{eq:mean_ratio_logn} to estimate the plug-in estimation of the quantile of order $p$:
\begin{equation}
  \hat{q}^{\mathrm{PI}}_p(\kk) = Q_{\UU}\left(\frac{m_Z(\kk, \UU)}{m_{Z^*}(\UU)}\exp\left(\frac{1}{2}\sigma^2_{\nu}(\kk, \UU)\right),p \right)
\end{equation}
...
By definition of the lognormal distribution, the logarithm of $Z/ Z^*$ is normally distributed,
\begin{equation}
  \label{eq:log_ratio}
  \log\left(\frac{Z(\kk, \uu)}{Z^*(\uu)}\right)\sim \mathcal{N}\left(m_{\nu}(\kk, \uu),\sigma_{\nu}^2(\kk, \uu)\right)
  \end{equation}

\paragraph{Reduction of the IMSE}

As we did for the random variable $Z - \alpha Z^*$, we can look to reduce at each iteration the integrated prediction variance of the ratio, which is $\sigma^2_{\nu}$ by defining the IMSE
\begin{equation}
  \IMSE(\mathcal{X}) = \int_{\Kspace\times\Uspace} \sigma_{\nu}^2(\kk, \uu) \,\mathrm{d}\kk\,\mathrm{d}\uu
\end{equation}
where $\sigma_\nu^2$ is constructed using $Z/Z^*$ and the design $\mathcal{X}$.
...
\begin{equation}
 \Ex_{Z(\kk,\uu)}\left[\IMSE(\mathcal{X} \cup \left\{\left((\kk, \uu),Z(\kk, \uu)\right)\right\}\right]
\end{equation}

meaning that $\sigma_\nu$ is computed using \cref{eq:log_ratio_eq}, and the GP augmented by the couple $\left((\kk, \uu), Z(\kk, \uu)\right)$.

\begin{figure}[ht]
  \centering
  \includegraphics{\imgpath qPI_aIMSE.pdf}
  \caption{\label{fig:qPI_aIMSE} augmented IMSE for the log ratio}
\end{figure}

% \begin{equation}
%   q_{p}^{-}(\kk) = Q_{\UU}\left(\right)
% \end{equation}
% \begin{equation}
%   q_{p}^{\mathrm{PI}}(\kk) = Q_{U}\left(\frac{m_Z(\kk, \UU)}{m_{Z^*}(\UU)};p\right)
% \end{equation}
...
%   \hat{\alpha}^{\mathrm{PI}}_p = \min_{\kk \in \Kspace} Q_U\left(\frac{m_Z(\kk, \UU)}{m_{Z^*}(\UU)};p\right) %=^{?} Q_{\UU}\left(\max_{\kk\in\Kspace} \frac{m_Z(\kk,\UU)}{m_{{Z^*}}(\UU)};p\right)
% \end{equation}

\paragraph{Sampling-based method, variation o QeAK-MCS}
...
\begin{equation}
  \log\frac{Z(\kk, \uu)}{Z^*(\uu)} \sim \mathcal{N}\left(m_{\nu}(\kk, \uu),\sigma^2_{\nu}(\kk, \uu)\right)
\end{equation}
and
\begin{align}
  q_p(\kk) = \exp Q_{\UU}\left(m_{\nu}(\kk, \UU), p\right)
\end{align}
Based on this expression the quantile, and the normal approximation of the log-ratio, we can get bounds of the true value of the quantile of order $p$ at a given $\kk\in\Kspace$:
\begin{align}
  \log q_p^{+}(\kk) = Q_{\UU}\left(m_{\nu}(\kk, \UU) + k \sigma_{\nu}(\kk, \UU), p\right) \\
  \log q_p^{-}(\kk) = Q_{\UU}\left(m_{\nu}(\kk, \UU) - k \sigma_{\nu}(\kk, \UU), p\right) \\
\end{align}
where $k = \Phi^{-1}(0.975)\approx 1.96$.
Let us define $\tilde{\kk}=\min_{\kk \in\Kspace} \hat{q}_p(\kk)$, that is the value which minimizes the quantile of order $p$ of the plugin estimate of the ratio.
By discretizing then the interval $\interval{q^+_p(\tilde{\kk})}{q^-_p(\tilde{\kk})}$, we get $\{q_l\}$ for $1\leq l \leq K_q$.

For each of these $K_q$ quantiles, we can follow the procedure of eAK-MCS, as described in the previous article, and
$\mathbb{M}(q_l) =\{(\kk, \uu) \mid m_{\nu}(\kk) - k\sigma_{\nu}(\uu) < q_l < m_{\nu}(\kk, \uu) + k\sigma_{\nu}(\kk, \uu)\}$

% Let $y_i$ be a sample path of $Z$, and $\kk\in\Kspace$. The estimation of the quantile of order $p$ is then:
% \begin{equation}
%   \label{eq:quantile_sample_path}
%   q_p^{(i)}(\kk) = \left(\frac{y_{i}(\kk, \uu)}{y^*_i(\uu)}\right)_{([Np])}
% \end{equation}

% As we assume that we can sample enough $\uu$ and compute cheaply the ratio based on the sample path, the estimation of~\eqref{eq:quantile_sample_path} can be considered accurate enough.

% However, the uncertainty in this estimation lies in the random nature of $Z$, which is used to generate the sample paths.
% We can however use multiple sample paths in order to compute a Monte-Carlo approximation of $q_p(\kk)$:
% \begin{equation}
...
% Before sampling trajectories however, one should first explore sufficiently the whole space $\Kspace \times \Uspace$ in order to get the prediction $m_Z$ and the prediction of the conditional minimum $m^*_{Z}$ of the GP should be strictly larger than $0$. This condition applies to the trajectories too.

% Let us say that we sampled $N$ function from $Z$, namely $y_i$ for $1 \leq i \leq N$. For each of these samples, we can get $q^{(i)}_p(\kk)$. Using Monte-Carlo, we can get a Monte-Carlo estimation of $q_p$:.
...
% \begin{figure}[ht]
%   \centering
%   \input{\imgpath quantile_estimation_GP.pgf}
%   \caption{\label{fig:quantile_estimation_GP} Estimation of $q_p$ based on the GP constructed on $J$}
% \end{figure}
...
% As done in \todo{add ref} to improve the estimation of $\Gamma_{\alpha}$, we can add points which reduce the uncertainty of the GP the most.
% \begin{figure}[ht]
%   \centering
...
% \begin{align}
%   \tilde{\kk} &= \argmax_{\kk\in \Kspace} \kappa(\kk) \label{eq:acq_candidate_IVPC}\\
