...
$\Gamma_{\beta}$ is then maximized with respect to its argument, in
...
we can define the family of additive-regret estimators.
  ...
  \begin{definition}[Additive-regret family of estimators]
    ...
    the maximizers of \cref{eq:def_gamma_additive}:
    \begin{equation}
...
can define an estimator as the value which maximizes this
...
\begin{definition}[Relative-regret family of estimators]
  \label{def:RR_family}
  Given $\alpha$, the value of $\kk$ that maximizes the probability of
...
\begin{figure}[ht]
  ...
  $\alpha$-acceptability]{\label{fig:gamma_alpha_increasing} Regions
    ...
    colored lines on the left plot are the boundaries of those
    regions}
\end{figure}
...
at maximizing a probability based on the regret, either additive or
relative.
\begin{table}[ht]
  \centering
  \begin{tabular}{ll}
