...
\begin{definition}[Relative-regret estimators family]
  Let $\alpha\geq 1$.
...
\end{definition}
$R_{\alpha}(\kk)$ is a \tocheck{measurable} subset of $\Uspace$, and by integrating this set with respect to $\Prob_\UU$, we get $\Gamma_{\alpha}(\kk)$, the probability of being $\alpha$-acceptable.
...
We have then that $\Gamma_1(\kk) % = \Prob_{\UU}\left[J(\kk, \UU) = J^*(\UU)\right]
