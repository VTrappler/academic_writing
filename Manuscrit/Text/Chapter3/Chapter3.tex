...
The resulting estimator does not take into account the uncertainty upon $\uu$, and can perform quite badly when the likelihood presents sharp ridges~\citep{berger_integrated_1999}.
...
One other approach, called Savage's maximin regret~\citep{savage_theory_1951} is to compare the current objective to the best performance given the uncertain variable $\uu$. The translated objective is called the \emph{regret} and is defined as
\begin{equation}
...
\begin{figure}[ht]
  ...
  \caption[Robust optimisation under uncertainty set]{\label{fig:decision_under_uncertainty} Illustration of global optimisation, worst-case, and regret worst-case. The red points on the contour of the cost function correspond to the minimisers of $J$ at each $\uu$ fixed}
\end{figure}
...
that $\kk \in \mathcal{I}_{\beta}(\uu)$, i.e.\
  ...
  \Cref{fig:gamma_beta_increasing} shows the regions of acceptability
