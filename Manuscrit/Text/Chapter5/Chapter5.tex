...
\begin{table}[!ht]
  ...
  \end{tabular}
  \caption[Types and sizes of each sediment class]{\label{tab:size_sediments} Type of sediments and size of the majority of particles for each type of sediment}
\end{table}
...
misspecification that we will consider as the environmental
variable. In this work, we chose to add a small multiplicative error
on the amplitude of the different components of the tide. Let
...
order to construct first a GP that can be used as a surrogate. This
will be done using the Python module
Scikit-learn~\cite{pedregosa_scikit-learn_2011}. Using similar notation
as in the previous chapter, we denote $\mathcal{X}_0$ the initial
design, and $Z$ the Gaussian Process constructed and fitted using
$\mathcal{X}_0$.  We will write
\begin{equation}
...
\todo{revoir figure, + transition minimiseurs à regrets based}
...
Maximising this approximation yields
\begin{equation}
...
defined as $\Delta_{\alpha}$. The $\mathrm{IMSE}$ associated with the
