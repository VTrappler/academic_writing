...
using Gaussian processes in~\cref{ssec:rr_estimators_croco}.
...
\Cref{fig:evo_alphap_kp} shows the maximal probability reached by
$\hat{\Gamma}^{\mathrm{PI}}_{\alpha}$ as a function of $\alpha$, and
the different components of the control variable
$\hat{\kk}_{\mathrm{RR},\alpha}$, all these quantities computed using
the metamodel $m_Z$ after the additional iterations.

\begin{figure}[ht]
  \centering
  \includegraphics{\imgpath palpha.png}
  \caption{\label{fig:p_alpha} Evolution of the maximal probability of
...
\begin{figure}[ht]
  ...
  \caption{\label{fig:evo_alphap_kp} Evolution of the maximum reached
    by $\hat{\Gamma}_{\alpha}$ when $\alpha$ increases, and associated
    estimates $\hat{\kk}_{\mathrm{RR},\alpha}$, using the GP enriched
    with 200 additional points minimising the augmented IMSE.}
\end{figure}

We can observe a gradual shift in those components when increasing the
threshold $\alpha$, especially for
$\hat{\kk}^{(1)}_{\mathrm{RR},\alpha}$. This suggests that setting
$\hat{\kk}^{(1)}$ equal to \num{-3.45} yields better values of the
objective function but with (relatively) low frequency, while choosing
\num{-3.4} is a more robust choice.
...
\end{align}
One main difference between the confidence interval given there, and
the one given~\cref{eq:max_gamma_alpha} is that the former translates
the uncertainty originating from the Gaussian Process, while the
latter accounts for the error in the \emph{sample average
  approximation} of the probability
$\hat{\Gamma}_{\alpha}^{\mathrm{PI}}$.
\begin{figure}[ht]
  \centering
  \includegraphics[]{\imgpath hatalphap.png}
  \caption{\label{fig:hat_alpha_p} Estimation of the threshold
