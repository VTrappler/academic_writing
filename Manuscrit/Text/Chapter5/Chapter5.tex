...
variables that parametrize the amplitude of some tidal components. \todo{finir intro}
...
error on the $S_2$ amplitude, $\UU_1$ and $\UU_2$ are independent and identically distributed, and $\UU \sim \mathcal{U}\left(\Uspace\right)$ with $\Uspace = \interval{0}{1}^2$.
...
We are going to estimate a member of the family of relative-regret estimators, as defined~\cref{def:RR_family}, on \cpageref{def:RR_family}, for a specific $\alpha\geq 1$.
\begin{equation}
...
We will first look to improve the estimation of $\Gamma_{\alpha}$, defined as the probability:
\begin{equation}
  \Gamma_{\alpha}(\kk) = \Prob_{\UU}\left[J(\kk,\UU) \leq \alpha J^*(\UU)\right]
\end{equation}
Using $Z$, the GP constructed on $\mathcal{X}_0$. We are going to reduce the augmented IMSE of the random process $\Delta_\alpha=Z - \alpha Z^*$, defined as
\begin{equation}
  \IMSE(\mathcal{X}) = \int_{\Kspace \times \Uspace} \sigma^2_{\Delta_{\alpha}}(x) \,\mathrm{d}x
\end{equation}
and
\begin{equation}
  x_{n+1} = \argmin_{(\kk,\uu)\in\Kspace\times \Uspace} \Ex_{Z(\kk, \uu)}\left[\IMSE(\mathcal{X})| \mathcal{X}_n \cup (\kk, \uu))\right]
\end{equation}
