...
layer at the bottom is equal to the size of the sediment, so the
rugosity is directly linked to the type of sediment found on the ocean
bed. \Cref{tab:size_sediments} presents a coarse classification, along
with the typical size of the sediments that can be found, that will
serve as a reference value, or \emph{truth value}
...
multiple order of magnitude. We are then going to define the control variable $\kk$ as
\begin{equation}
  \kk = \log \zob
\end{equation}
...
\begin{figure}[ht]
  \centering
  \includegraphics{/home/victor/optimisation_dahu/optim_sediments/map_log_399.png% /home/victor/optimisation_dahu/optim_true/map_150.png
  }
  \caption{\label{fig:optimization_map_399} Optimization of $\zob$ on
    the whole space using gradient obtained via adjoint method, after
    $400$ iterations.}
...
well retrieved. % In terms of sediments, the bottom of the continental shelf
...
control.% , which is probably due
           %            to their size, \emph{i.e.} their truth value is significantly
           %            larger than the rest of the sediments, and they are sparsely
           %            distributed.
...
\Kspace = \interval{-11}{-3}^6% \text{ and } \kk_i = \log \zob^i
\end{equation}
...
\begin{equation}
  \kk=\left(\kk_1,\dots,\kk_6\right) \in \Kspace, \quad \kk_i = \log(\zob^i), \quad \KK \in \Kspace = \interval{-11}{-3}^6
\end{equation}
and the uncertain variale is defined as
\begin{equation}
...
\end{equation}
whith $C_Z((\kk, \uu),(\kk, \uu)) = \sigma^2_Z(\kk, \uu)$. We have for
...
relative-regret is not defined. Alternatively\todo{réécrire}, this
violates the assumption of the positivity of $Z^*(\uu)$
in~\cref{ssec:lognormal_approx}.

To avoid these issues, we are first going to enrich the design near
...
\begin{equation}
  J^* : \uu\in\Uspace \mapsto J^*(\uu) = \min_{\kk\in\Kspace} J(\kk, \uu)
\end{equation}
and the conditional minimiser function as
\begin{equation}
  \kk^*:\uu \in \Uspace \mapsto \kk^*(\uu) = \argmin_{\kk \in \Kspace} J(\kk, \uu)
\end{equation}
