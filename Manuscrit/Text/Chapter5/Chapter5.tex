...
\begin{table}
  ...
  \begin{tabular}{rrl}
    \toprule
    Method & Quantity of interest & Required \# of GP predictions\\ \midrule
    augmented IMSE & Integral in $(1 + \dim \Kspace \times \Uspace)$ dimensions& $\propto N_{\int}\cdot N_{*}$\\
    Qe-AK MCS & Probability of coverage of $\mathbb{M}_{\eta}(\mathfrak{q})$ & $\propto \mathrm{Vol}(\mathcal{M}_{\eta})^{-1} \cdot N_{*}$\\
    \bottomrule
  \end{tabular}
  \caption{Results}
