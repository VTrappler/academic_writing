...
$\kk^{(1) *}_Z(\UU)$ and $\kk^{(2) *}_Z$ for instance, indicating that
an increase in one of those value of the friction is compensated by
the decrease in the other component.  \todo{finir commentaire figure}
We can also notice that the marginal distribution of the first
component $\kk_Z^{(1)*}(\UU)$ exhibits two modes: one at \num{-3.6},
and one at around \num{-3.45}.
\begin{figure}[ht]
  \centering
...
for all $\alpha >1$: $\hat{\kk}^{(1)}_{\mathrm{RR},\alpha}$ seems to
increase with respect to $\alpha$, from \num{-3.45} to
\num{-3.38}. For $\kk^{(1)}$, all these values are slightly higher
than $\hat{\kk}^{(1)}_{\mathrm{opt}}$ found when optimising globally,
in~\cref{tab:ktruthkopt},~\cpageref{tab:ktruthkopt}. For the second
and third components, we can see that $\hat{\kk}_{\mathrm{RR},\alpha}$
shows lower values than $\hat{\kk}_{\mathrm{opt}}$.

\begin{figure}[ht]
  \centering
  %\input{\imgpath evo_alpha_p_kp.pgf}
  \includegraphics{\imgpath evo_alpha_p_kp_bnd.png}
  \caption{\label{fig:evo_alphap_kp} Components of
...
to Lebesgue's measure on $\Kspace \times \Uspace$), along with the
95\% confidence intervals associated with its estimation. We can see
that first, its volume increases slightly. This is can be seen as an
exploration phase, where the additional evaluations change the
hyperparameters of the GP, and affect the estimation of the
conditional minimums. After enough iterations, the volume of the
margin decreases, as points are added while the hyperparameters stay
quite the same.

\begin{figure}[ht]
  \centering
  \includegraphics{\imgpath volume_margin_uncertainty.png}
  \caption{\label{fig:vol_Meta} Volume of the margin of uncertainty
    associated with $\mathfrak{q}_2 = \hat{\alpha}_p$, and 95\%
    confidence intervals of its estimation}
\end{figure}


Using the GP conditioned on the final design, for $op=0.95$ we have
\begin{align}
  \hat{\alpha}_p &= \num{1.3791} \\
...
\end{figure}
Similarly as in~\cref{fig:evo_alphap_kp}, we can see that the first
component of $\hat{\kk}_{\mathrm{RR}, \alpha_p}$ is slightly larger
than the one of $\hat{\kk}_{\mathrm{opt}}$. For the third component as
well, the relative-regret estimates shows lower values than the optimal ones.
