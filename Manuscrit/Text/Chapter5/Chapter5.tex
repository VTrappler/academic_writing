...
no amplitude is perturbated: $\tilde{A}_k(\uu^{\mathrm{truth}}) = A_k$
for $1\leq k \leq 5$.
...
$\uu^{\mathrm{truth}}=(\num{0.5},\num{0.5}) \neq
\hat{\uu}_{\mathrm{opt}}$.  This can be explained by the choice of the
control variable, which possesses 3 elements, while the truth used to
generate the observations had 6. The difference
$\hat{\uu}^{\mathrm{opt}}$ and $\uu^{\mathrm{truth}}$ happens in order
to compensate for the difference of dimensionality between
