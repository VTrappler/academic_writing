...
\Cref{fig:SA_tides} shows the Sobol indices of first order (left), second
...
We are here going to estimate members of the relative-regret family of estimators as defined~\cref{def:RR_family}, on
...
As $\mathrm{RR}$ is unknown directly, we can also apply the plug-in approach, and define
\begin{equation}
...
In this section, we will fix a level of confidence $p=0.95$, and look to minimise the quantile of the ratio defined in~\cref{eq:RRPI}.
By definition, $\kk_{\mathrm{RR},\alpha_p}$ and $\Gamma_{\alpha_p}$  verifies
\begin{equation}
