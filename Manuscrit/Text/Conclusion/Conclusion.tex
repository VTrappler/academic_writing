...
\todo{refaire 2derniers par}
% Choice of quantile ? to remove maybe


In this thesis, the random variables $J^*(\UU)$ which represent the
conditional minimum, and the associated minimiser $\kk^*(\UU)$ play a
central role in our notions of robustness. Those distribution are
involved in the expression of the regret, that we control through its
quantile.% However, we do not control the values of the function when
