...
\todo{to write}
We are first going to focus on $1D$ covariance functions, as 
\label{sec:cov_fun}
\begin{itemize}
\item Desired properties
  \begin{itemize}
  \item stationarity
    One important property usually assumed is the stationarity of the covariance. This property implies that the covariance function for two points $x$ and $x^{\prime}$ is a function of their difference, that is $C(x,x^{\prime}) = \gamma(x - x^{\prime})$.
    Moreover,
    \begin{align}
      \gamma(0) \geq \gamma(h) \forall h \\
      \gamma(h) = \gamma(-h)\\
      \gamma \text{ decreases when } h\geq 0 \text{ increases} 
    \end{align}
    This properties encodes the fact that knowledge on a specific point improves the knowledge on its neighbourhood, in either directions.
  \item semi-definite positiveness
  \end{itemize}
\item parametric models of covariance
\item examples
  
\item In the multidimensional case, the 
...
\begin{align}
  \max_{\kk} \Gamma_{\alpha_p}(\kk) = \Gamma_{\alpha_p}(\kk_p) = \Prob_{U}\left[J(\kk_p,\UU) \leq \alpha_p J^*(\UU)\right] = p
\end{align}
...
\begin{figure}[!h]
  \centering
  \scalebox{0.8}{\input{/home/victor/acadwriting/ocean_floor.pgf}}
